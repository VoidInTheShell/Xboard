...
Pushes to Xboard `master` deploy production automatically through the GitHub `production` Environment. Manual dispatch with `deployment_target=production` is also supported; selecting staging cannot reach JPGREEN. Production deploys its run-specific `build-<run_id>-<run_attempt>` Xboard image and retains the currently installed Theme/Admin version tags unless explicitly overridden. This prevents a backend release from undoing a newer frontend release. The workflow connects as a dedicated `xboard-ci` SSH principal whose key is bound to a root-owned forced-command dispatcher. The account is not a Docker-group member and cannot open a general shell, upload arbitrary executables, forward ports or run arbitrary sudo commands.

Install the matching trusted host scripts and `release-maintenance.py` before enabling the updated workflow. The `current` component selector resolves to the installed version tag while holding the shared deployment lock. Pulls, migrations and all three container health checks remain within the release transaction; failure restores the database and previous runtime files. Cleanup after success retains three successful release snapshots and referenced images, skips incident/failed backups, and never prunes persistent volumes or unrelated repositories.
...
Ordinary mainline deployments preserve the SQLite database, uploads, themes, plugins, application `.env`, Redis state and runtime credentials. With the App stopped, the trusted deployer uses SQLite's online backup API to create a verified, WAL-safe release snapshot, applies migrations, and compares protected business-table counts before it starts the new App. If a migration fails or any protected count decreases, it restores the snapshot and previous runtime files, brings the previous App back up, and leaves the release red. A fresh-data deployment requires a manual workflow dispatch, the boolean reset input, the exact confirmation text `RESET JPGREEN XBOARD DATA`, and a separate root-only authorization file at `/etc/xboard-ci/reset-authorizations/<master-sha>`. The dispatcher consumes that file only for the corresponding SHA and restores it when deployment fails, so possession of the CI key alone cannot erase production data.
